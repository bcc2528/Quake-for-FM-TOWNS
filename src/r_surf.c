...
// bound, invert, and shift
	for (i=0 ; i<size ; i++)
	{
		...
		blocklights[i] = _max((65280 - (int)blocklights[i]) >> 2, 64);
	}
}
...
// from a source range of 0 - 255
	...
	texwidth = mt->width >> surfmip;

	blocksize = 16 >> surfmip;
	blockdivshift = 4 - surfmip;
...
//==============================
	...
	if (r_pixbytes == 1)
	{
		pblockdrawer = surfmiptable[surfmip];
	...
	smax = mt->width >> surfmip;
	twidth = texwidth;
	tmax = mt->height >> surfmip;
...
// << 16 components are to guarantee positive values for %
	...
	basetptr = &r_source[((((basetoffset >> surfmip) 
		+ (tmax << 16)) % tmax) * twidth)];
...
void R_DrawSurfaceBlock8_mip3 (void)
{
	...
	// FIXME: use delta rather than both right and left, like ASM?
		...
		for (i=0 ; i<2 ; i++)
		{
			...
			prowdest[1] = ((unsigned char *)vid.colormap)
			[(lightright & 0xFF00) + psource[1]];
			prowdest[0] = ((unsigned char *)vid.colormap)
			[((lightright + lightstep) & 0xFF00) + psource[0]];

...
void R_EmitEdge (mvertex_t *pv0, mvertex_t *pv1)
{
	...
	// FIXME: build x/yscale into transform?
		u0 = _min(_max((xcenter + (xscale * lzi0) * transformed[0]),
			r_refdef.fvrectx_adj), r_refdef.fvrectright_adj);
	
		v0 = _min(_max((ycenter - (yscale * lzi0) * transformed[1]),
			r_refdef.fvrecty_adj), r_refdef.fvrectbottom_adj);
...
// transform and project
	...
	r_u1 = _min(_max((xcenter + (xscale * r_lzi1) * transformed[0]),
			r_refdef.fvrectx_adj), r_refdef.fvrectright_adj);

	r_v1 = _min(_max((ycenter - (yscale * r_lzi1) * transformed[1]),
			r_refdef.fvrecty_adj), r_refdef.fvrectbottom_adj);
